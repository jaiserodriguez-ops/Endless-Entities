...
namespace Basic.Triggers
{
    ...
    {
        [Header("(Handles both 3D and 2D)\n\nBehavior (Trigger uses ''Player'' tag)")]
        ...
        {
            await TriggersEnterLogic(other);
        }
        ...
        private async Task TriggersEnterLogic(Collider other)
        {
        ...
        private async Task TriggersEnterLogic(Collider2D other)
        {
            if (other.CompareTag(_compareTag) && !_doNotUseAnyTag)
            {
                await LoadSceneSequenceAsync();
